      ...
      if (ssoAccount) {
        if (!req.user || req.user.id === ssoAccount.user.id) {
          ...
          const toast = {
            type: 'error',
            message: {
              en: 'It is not allowed to connect with a social account which is already connected with another account. Please delete that account first and try again.',
              ko: '이미 다른 계정에 연결되어 있는 소셜 계정과 연결할 수 없습니다. 소셜 계정과 연결된 계정을 탈퇴한 후 다시 시도해 주세요.',
        ...
        if (!req.user) {
          ...
          if (otherSSOAccountCount > 0) {
            const toast = {
              type: 'error',
              message: {
                en: 'It is not allowed to connect with two accounts with the same provider together.',
                ko: '같은 제공자가 제공하는 소셜 계정은 한 개만 추가할 수 있습니다.',
              }[req.user?.language ?? 'en'],
            };
            req.session.toast = JSON.stringify([
              ...(req.session.toast ? JSON.parse(req.session.toast) : []),
